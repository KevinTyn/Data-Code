...
NODATA_VALUE = -9999.0
eps_w = 1e-6
OUT_DIR = r"H:\InSAR_Mosaic_Project2"
...
COH_THRESHOLD = 0.75  # 强制要求的严格相干性阈值
...
class InSARMSTMosaic:
    ...
    def _calculate_robust_params(self, idx1, idx2):
        ...
        with rasterio.open(img1['vel']) as src1, rasterio.open(img2['vel']) as src2, \
            ...
            # --- Mask Selection ---
            # [修改] 强制只用相干性 > 0.75 的像元，去除一切降级选择逻辑
            mask_valid = (c1_arr > COH_THRESHOLD) & (c2_arr > COH_THRESHOLD) & np.isfinite(v1_arr) & np.isfinite(v2_arr)
            cnt = int(np.sum(mask_valid))

            if cnt < self.min_points_thresh:
                logging.debug(
                    f"  {pair_tag}: Rejected. Points > {COH_THRESHOLD}: {cnt} (Need: {self.min_points_thresh})")
                return None, 0

            logging.debug(f"  {pair_tag}: Accepted Mask (Strict > {COH_THRESHOLD}), Points: {cnt}")
    ...
    def generate_mosaic(self, output_path_vel, output_path_coh=None):
        ...
        for idx, img in enumerate(self.images):
            ...
            with rasterio.open(img['vel']) as src, rasterio.open(img['coh']) as src_c:
                ...
                # [修改] 严格要求相干性 > 0.75 才参与拼接
                mask = np.isfinite(data) & (data != NODATA_VALUE) & (coh > COH_THRESHOLD)
