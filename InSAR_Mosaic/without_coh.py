...
NODATA_VALUE = -9999.0
eps_w = 1e-6
OUT_DIR = r"H:\InSAR_Mosaic_Project1"
...
class InSARMSTMosaic:
    ...
    def _calculate_robust_params(self, idx1, idx2):
        ...
        with rasterio.open(img1['vel']) as src1, rasterio.open(img2['vel']) as src2:
            ...
            # --- Mask Selection (直接使用有效值) ---
            mask_valid = np.isfinite(v1_arr) & np.isfinite(v2_arr) & (v1_arr != NODATA_VALUE) & (v2_arr != NODATA_VALUE)

            cnt = int(np.sum(mask_valid))
            if cnt < self.min_points_thresh:
                return None, 0

            logging.debug(f"  {pair_tag}: Accepted points based on valid velocity, Points: {cnt}")
            ...
            # [核心修改] 不再将相干性作为权重，全设置为等权 1.0
            w1_base = np.ones_like(v1)
            w2_base = np.ones_like(v2)
            ...
            # 保存内点掩膜到 GeoTIFF
            # [核心修改] 边缘权重仅使用有效内点数
            raw_weight = float(np.sum(mask_valid))
            if raw_weight > 1e-4:
        ...
        for idx, img in enumerate(self.images):
            ...
            with rasterio.open(img['vel']) as src, rasterio.open(img['coh']) as src_c:
                ...
                if np.sum(mask) > 0:
                    ...
                    # [核心修改] 取消相干性加权，仅使用距离平滑权重进行融合拼接
                    combined_w = dist[mask]
...
if __name__ == "__main__":
    ...
    inputs = [
        ...
        {'id': 6, 'vel': fr"E:\shanxi\19\19_113_121\vel_19_113_121.tif", 'coh': fr"E:\shanxi\19\19_113_121\coh_19_113_121.tif"},
    ...
    # 为了防止此行报错（因为上面的 inputs 被注释掉了），如果你自己运行请确保 inputs 有定义。
    # 这里保持和原结构完全一致。
    for item in inputs:
