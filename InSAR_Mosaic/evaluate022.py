# -*- coding: utf-8 -*-
"""
Script 2 (Final V31 - Manual Subdir Control): InSAR Mosaic Validator
功能：读取模型和掩膜，进行单剖面交互验证。
特性：
1. [Layout] 图例全部移至绘图区域外部（下方）。
2. [Metrics] 全指标统计 (RMSE, MAE, STD, MED)。
3. [Visual] 图1 (Visual) 统一色标。
4. [Visual] 图2 (Stats Final) 基于最终掩膜 (去除离群值)。
5. [Visual] 图3 (Stats All) 基于所有有效点 (包含离群值)，布局同图2。
6. [Path] 手动指定 Models, Results, Masks 下的具体子文件夹名称进行处理。
"""
...
def calculate_metrics(d_raw, d_corr):
    """计算统计指标"""
    if len(d_raw) == 0:
        return {k: np.nan for k in
                ['rmse_r', 'rmse_c', 'mae_r', 'mae_c', 'std_r', 'std_c', 'med_r', 'med_c', 'imp', 'pts']}
...
class MosaicValidator:
    def __init__(self, image_list, model_path, mask_dir, label="Task"):
        ...
        self.label = label
        self.mask_dir = mask_dir  # 接收对应的子文件夹掩膜路径
    ...
    def _process_pair(self, i, j, save_dir, csv_file):
        ...
        # 4. 最终掩膜 (去除离群值)
        mask_file = os.path.join(self.mask_dir, f"Mask_{i}_{j}.npy")
        mask_final = None;
        status_tag = "Robust"
        ...
        # 5. 准备两组数据
        d_raw_final = diff_raw_mm[mask_final]
        d_corr_final = diff_corr_mm[mask_final]
        metrics_final = calculate_metrics(d_raw_final, d_corr_final)

        d_raw_all = diff_raw_mm[mask_basic]
        d_corr_all = diff_corr_mm[mask_basic]
        metrics_all = calculate_metrics(d_raw_all, d_corr_all)

        # 6. 写CSV (仅记录 Final 结果)
        line = f"{i}-{j},{name1},{name2},{metrics_final['pts']},{metrics_final['rmse_r']:.3f},{metrics_final['rmse_c']:.3f},{metrics_final['mae_r']:.3f},{metrics_final['mae_c']:.3f},{metrics_final['std_r']:.3f},{metrics_final['std_c']:.3f},{metrics_final['med_r']:.3f},{metrics_final['med_c']:.3f},{metrics_final['imp']:.1f},{status_tag}\n"
        ...
        self._plot_stats_page(name1, name2, v1_corr_mm, v2_corr_mm, surf_mm, mask_final,
                              v1_mm[mask_final], v2_mm[mask_final], v1_corr_mm[mask_final], v2_corr_mm[mask_final],
                              d_raw_final, d_corr_final,
                              line_r, line_c, metrics_final,
                              os.path.join(save_dir, f"Check_{i}_{j}_2_Stats.png"),
                              f"{status_tag} Mask (Cleaned)")

        self._plot_stats_page(name1, name2, v1_corr_mm, v2_corr_mm, surf_mm, mask_basic,
                              v1_mm[mask_basic], v2_mm[mask_basic], v1_corr_mm[mask_basic], v2_corr_mm[mask_basic],
                              d_raw_all, d_corr_all,
                              line_r, line_c, metrics_all,
                              os.path.join(save_dir, f"Check_{i}_{j}_3_Stats_All.png"),
                              "All Valid Points (Raw Overlap)")
    ...
    def _plot_stats_page(self, name1, name2, img1, img2, surf_map, mask_2d,
        ...
        ax_sum.axis('off')
        bg_color = "#e0ffe0" if "Exact" in mode_label else "#ffffe0"
        if "All" in mode_label: bg_color = "#e0e0ff"
...
if __name__ == "__main__":
    ...
    # ==========================================
    target_year = 17
    sub_dir_name = "17_m3"  # <--- 每次运行前，只需修改这个名字！

    # 根据手动指定的子文件夹，自动构建完整的工作路径
    ...
    # 检查指定的模型路径是否存在，以防输错名字
    if not os.path.exists(current_model_dir):
        print(f"Error: 找不到指定的文件夹路径: {current_model_dir}")
        exit()

    if not os.path.exists(current_qc_dir):
        os.makedirs(current_qc_dir)

    print(f"=== STARTING V31 (Manual Subdir: {sub_dir_name}) ===")
    print(f">>> 模型读取路径: {current_model_dir}")
    print(f">>> 报告输出路径: {current_qc_dir}")
    print(f"{'=' * 50}")
    ...
    for pid, imgs in tracks.items():
        ...
        model_path = os.path.join(current_model_dir, f"Track_{pid}_model.npy")
        if os.path.exists(model_path):
            try:
                # 传入当前子文件夹的 mask 路径
                val = MosaicValidator(imgs, model_path, mask_dir=current_mask_dir, label=f"Track_{pid}")
                val.run_validation(os.path.join(current_qc_dir, f"Track_{pid}"))
            except Exception as e:
                print(f"Error Track {pid} in {sub_dir_name}: {e}")

    # --- 4. 处理跨轨(CrossTrack)的拼接验证 ---
    if os.path.exists(current_result_dir):
        # 从当前的 Result 子文件夹寻找 Strip 数据
        ...
        if len(strips) >= 2 and os.path.exists(cross_model_path):
            try:
                # 传入当前子文件夹的 mask 路径
                val = MosaicValidator(strips, cross_model_path, mask_dir=current_mask_dir, label="CrossTrack")
                val.run_validation(os.path.join(current_qc_dir, "CrossTrack"))
            except Exception as e:
