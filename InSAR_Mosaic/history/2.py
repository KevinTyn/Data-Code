...
# =========================================================
# 1. ALS 模型强制开关
# None: 保持原有的自适应双重保险逻辑 (自动在 const 和 linear 间判断)
# 'const': 强制所有重叠区只使用常数项 (Constant) 进行平差，无论同轨异轨
# 'linear': 强制所有重叠区使用一次空间平面 (Linear) 进行平差，无论同轨异轨
# 'quadratic': 强制所有重叠区使用二次空间曲面 (Quadratic) 进行平差，无论同轨异轨
FORCE_ALS_MODEL = 'linear'

# 2. RANSAC 空间拟合开关
# True: 开启 RANSAC 空间拟合 (原有逻辑)
# False: 关闭 RANSAC (跳过空间拟合，直接使用 ALS 的平差结果)
USE_RANSAC = False
# =========================================================
...
class InSARMSTMosaic:
    ...
    def _calculate_robust_params(self, idx1, idx2):
        ...
        with rasterio.open(img1['vel']) as src1, rasterio.open(img2['vel']) as src2, \
            ...
            # ===========================================================
            # [核心逻辑] 强制开关 OR 双重保险：绝对精度检查 + 相对提升检查
            # ===========================================================

            if FORCE_ALS_MODEL in ['const', 'linear', 'quadratic']:
                # 若开启了强制开关，则直接接管，跳过原有自适应检查
                chosen_model = FORCE_ALS_MODEL
                audit_log = f"[SWITCH] Forced ALS Model to '{FORCE_ALS_MODEL}'"
            else:
                ...
                if chosen_model == 'linear':
                    ...
                    # 如果 Const 的误差本来就很小 (<3mm)，根本不需要 Linear
            ...
            if chosen_model == "const":
                ...
                # 注意：即便选择了 quadratic，强度的 1D 映射依然使用线性平差，
                # 将二次曲面的拟合完全交给下方的空间 RANSAC，以保证数值稳定性。
                t = t_initial
            ...
            # [修改] RANSAC 智能锁轴策略，增加二次曲面 (Quadratic) 逻辑
            if USE_RANSAC and chosen_model != "const" and point_count > self.min_points_thresh:
                ...
                if np.sum(mask_diff) > self.min_points_thresh:
                    ...
                    # --- 根据 chosen_model 自动扩展设计矩阵 A ---
                    if chosen_model == 'quadratic':
                        # 二次项拟合：[1, X, Y, X^2, Y^2, XY]
                    ...
                    try:
                        ...
                        if inlier_count > self.min_points_thresh // 2:
                            ...
                            # sp_params 顺序: [Offset, SlopeX, SlopeY, px^2, py^2, px*py]
            ...
            # 如果没跑 RANSAC (如 const 或 USE_RANSAC=False)，则用 mask_valid 作为最佳掩膜
