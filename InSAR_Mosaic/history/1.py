...
# =========================================================
# --- [新增功能开关：完全解绑 ALS 与 RANSAC] ---
# =========================================================
# 1. ALS 模型开关 (处理数值域 V1/V2 的关系)
# 'const' : 强制仅计算常数平移 (Scale 强制为 1.0) -> 最符合 InSAR 物理意义
# 'linear': 强制计算平移 + 比例缩放
# None    : 保持原代码的自适应判定逻辑
ALS_MODEL = 'linear'

# 2. RANSAC 空间拟合阶数 (处理空间域 X/Y 的误差分布)
# 0: 关闭空间拟合 (仅使用 ALS 结果)
# 1: 拟合一次空间平面 (倾斜坡度: X, Y)
# 2: 拟合二次空间曲面 (抛物面弯曲: X^2, Y^2, XY)
# None: 随原代码逻辑 (若 ALS 判定为 const 则为 0，若为 linear 则为 1)
RANSAC_ORDER = 1
# =========================================================
...
class InSARMSTMosaic:
    ...
    def _calculate_robust_params(self, idx1, idx2):
        ...
        with rasterio.open(img1['vel']) as src1, rasterio.open(img2['vel']) as src2, \
            ...
            auto_model = best_point["model"] if best_point else "linear"
            ...
            # ===========================================================
            # [解绑核心逻辑] 独立判断 ALS 和 RANSAC 模式
            # ===========================================================

            # 1. 确定 ALS 模型 (处理 Scale)
            if ALS_MODEL in ['const', 'linear']:
                actual_als_model = ALS_MODEL
                audit_log += f"[ALS Forced: {ALS_MODEL}] "
            else:
                # 触发原有的自适应双重保险
                actual_als_model = 'linear'
                if auto_model == 'quadratic': auto_model = 'linear'
                if auto_model == 'linear':
                    ...
                    if self.mosaic_mode == 'same_track':
                        if is_const_good_enough:
                            actual_als_model = 'const'
                            audit_log += f"[AUDIT] VETO: Const Good (<{RMSE_TOLERANCE}). {audit_details} "
                        elif not is_significant_impv:
                            actual_als_model = 'const'
                            audit_log += f"[AUDIT] VETO: Low Impv (<{IMPROVEMENT_THRESH:.0%}). {audit_details} "
                        elif not is_safe_points:
                            actual_als_model = 'const'
                            audit_log += f"[AUDIT] VETO: Low Pts. {audit_details} "
                        else:
                            audit_log += f"[AUDIT] KEEP Linear (Nec). {audit_details} "
                    else:
            ...
            # 2. 确定 RANSAC 阶数 (处理空间曲面拟合)
            if RANSAC_ORDER in [0, 1, 2]:
                actual_ransac_order = RANSAC_ORDER
                audit_log += f"[RANSAC Forced: Order {RANSAC_ORDER}]"
            else:
                # 随动原逻辑：ALS为const则关RANSAC(0)，为linear则开平面RANSAC(1)
                actual_ransac_order = 0 if actual_als_model == 'const' else 1
                audit_log += f"[RANSAC Auto: Order {actual_ransac_order}]"

            logging.info(f"  {pair_tag} [{point_count} pts]: {audit_log} | lambda={best_lambda:.2f}")

            # --- ALS 迭代 (IGG III) ---
            igg_stats_total = [0, 0, 0]  # Down, Cut, Total

            if actual_als_model == "const":
            ...
            if actual_ransac_order > 0 and point_count > self.min_points_thresh:
                ...
                if np.sum(mask_diff) > self.min_points_thresh:
                    ...
                    try:
                        ...
                        if inlier_count > self.min_points_thresh // 2:
                            ...
                            elif fit_mode == "XY-Full":
                                sp_params[1] = res[1]  # SlopeX
                                sp_params[2] = res[2]  # SlopeY
                            elif fit_mode == "Quadratic-Full":
                                sp_params[1] = res[1]  # SlopeX
                                sp_params[2] = res[2]  # SlopeY
                                sp_params[3] = res[3]  # px^2
                                sp_params[4] = res[4]  # py^2
                                sp_params[5] = res[5]  # px*py
